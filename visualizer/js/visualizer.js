...
const Visualizer = {
  ...
  config: {
    ...
    fleetColor: [
      "rgba(64,080,080,1.00)",
      "rgba(192,000,000,1.00)",
      "rgba(112,160,240,1.00)",
    ],
    planetColor: [
      "rgba(64,080,080,1.00)",
      "rgba(192,000,000,0.40)",
      "rgba(112,160,240,0.40)",
    ],
  },
  ...
  drawFrame: function (frame) {
    ...
    if (frameNumber >= this.moves.length) frameNumber = this.moves.length - 1;
    ...
    for (let i = 0; i < this.planetZOrder.length; i++) {
      ...
      const highlight_this_planet =
        this.active_planet >= 0 && this.active_planet === z;
      ...
      ctx.arc(
        display_x + 0.5,
        this.canvas.height - display_y + 0.5,
        this.config.planet_pixels[planet.growthRate] + 1,
        0,
        Math.PI * 2,
        true
      );
      ...
      ctx.arc(
        display_x,
        this.canvas.height - display_y,
        this.config.planet_pixels[planet.growthRate],
        0,
        Math.PI * 2,
        true
      );
      ...
      if (highlight_this_planet) {
        ...
        ctx.fillText(
          ...
          display_x,
          this.canvas.height - display_y + 10
        );
      } else {
        ctx.fillText(
          planet.numShips,
          display_x,
          this.canvas.height - display_y + 5
        );
      }
    }
    ...
    for (let i = 0; i < fleets.length; i++) {
      ...
      const progress =
        (fleet.progress + 1 + (frame - frameNumber)) / (fleet.tripLength + 2);
      fleet.x =
        fleet.source.x + (fleet.destination.x - fleet.source.x) * progress;
      fleet.y =
        fleet.source.y + (fleet.destination.y - fleet.source.y) * progress;
      ...
      this.dirtyRegions.push([
        display_x - 25,
        this.canvas.height - display_y - 35,
        60,
        60,
      ]);
    }
    ...
    $(".player1Name a").css({
      color: Visualizer.config.teamColor[1],
    ...
    $(".player2Name a").css({
      color: Visualizer.config.teamColor[2],
      "text-decoration": "none",
    });
  },
  ...
  run: function () {
    ...
    if (this.frameDrawEnded != null) {
      this.frame +=
        ((this.frameDrawStarted - this.frameDrawEnded) / 1000) *
        this.turnsPerSecond;
    }
    this.drawFrame(this.frame);
    this.frameDrawEnded = new Date().getTime();
    this.frame +=
      ((this.frameDrawEnded - this.frameDrawStarted) / 1000) *
      this.turnsPerSecond;
  ...
  setFrame: function (targetFrame) {
    this.frame = Math.max(
      0,
      Math.min(this.moves.length - 1, Math.floor(targetFrame))
    );
  },
  ...
  parseData: function (input) {
    ...
    for (let i = 0; i < turns.length; i++) {
      ...
      move.planets = turn
        .slice(0, this.planets.length)
        .map(ParserUtils.parsePlanetState);
...
(function ($) {
  ...
  const updateMove = function (evt) {
    const chart = $("#chart");
    const move =
      (Math.max(200, Visualizer.moves.length) *
        (evt.pageX - chart.offset().left)) /
      chart.width();
  ...
  display.bind("drawn", function () {
    $("#turnCounter").text(
      "Turn: " +
        Math.floor(Visualizer.frame) +
        " of " +
        (Visualizer.moves.length - 1)
    );
  });

  $("#error_message")
    .text(Visualizer.error_message)
    .css({ color: Visualizer.config.teamColor[1] });
  ...
  $(".playerVs").text("v.s.");
  $("title").text(
    Visualizer.players[0] + " v.s. " + Visualizer.players[1] + " - Planet Wars"
  );
